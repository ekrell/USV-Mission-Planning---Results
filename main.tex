...
%-------------- Mission Planner Fully Connected -----
\begin{algorithm}%
...
% {Evan, please take a look at the Algorithm \ref{algo:algorithm}. Try to write your algorithms using a similar format. Delete when you are done!}
% {Okay, thanks: I will.}
% OK, thanks

% \begin{algorithm}
% \caption{DP-based Approach for TSP-CPP}
% \label{algo:algorithm}
% \SetAlgoVlined
% %\SetAlgoLined
%  \KwIn{Depot $v_0$, vertices of the regions in $\mathcal{N}_0$, and sensor range $r\times r$ of the UAS. }
%  \KwOut{A shortest path that covers all regions in $\mathcal{N}_0$ starting and ending at depot $v_0$, and its length.}
% %Initialize $D(T, i, c_{ij})$ with $\infty$, for all $T \subseteq \mathcal{N}_0$, $i \in T$, and $j \in A$\; 
% \ForEach{$i \in \mathcal{N}_0$}{
% \ForEach{$q \in A$}{
% $S_i \leftarrow f(c_{iq})$\;
% $D(\{i\}, i, c_{iq}) \leftarrow d(v_0, s_{in_i}) + \sum_{m = 1}^{n_i-1}d(s_{im}, s_{i(m+1)})$\;
% $P(\{i\}, i, c_{iq}) \leftarrow (S_i, v_0)$\;
% }
% }
% \ForEach{$t = 2, \ldots, N$}{
% \ForEach{$T \subseteq \mathcal{N}_0$ where $|T| = t$}{
% \ForEach{$i \in T $}{
% \ForEach{$q \in A $}{
% \ForEach{$k \in T-\{i\} $}{
% \ForEach{$l \in A $}{
% $S_i \leftarrow f(c_{iq})$\;
% $g(c_{kl},c_{iq}) \leftarrow d(c_{kl}, s_{in_i}) + \sum_{m = 1}^{n_i-1}d(s_{im}, s_{i(m+1)})$\;
% $dist \leftarrow D(T-\{i\}, k, c_{kl}) + g(c_{kl}, c_{iq})$\;
% \If{$dist < D(T, i, c_{iq}) $}{
% $D(T, i, c_{iq}) \leftarrow dist$\;
% $P(T, i, c_{iq}) \leftarrow (S_i, c_{kl})$\;
% }
% }
% }
% }
...
% }
% }
% \Return{$\min\limits_{i \in \mathcal{N}_0, q \in A} D(\mathcal{N}_0, i, c_{iq}) + d(v_0, c_{iq})$ and the shortest path by backtracking over arcs in table $P$.}
% \end{algorithm} 
%-----------------------------------------------
...
A mission planner was developed that used a heuristic to select the order of targets to visit before actually planning the segments. This means that the expensive invocation of multiple planners for each possible mission is avoided. The straight-line distance between target centers was used in place of \textit{Goto} planning, and \texit{Coverage} planning was done only once per region with arbitrary region entrance and exit paths. Relaxing the dependencies that dramatically increase mission planning complexity was able to approximate the result of exhaustive search. Mission planning time was reduced from ~24 hours (120 possible missions, evaluated in parallel using 15 threads) to a under 10 minutes in serial. 
